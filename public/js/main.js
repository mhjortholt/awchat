...
function start() {
  ...
  navigator.mediaDevices
      ...
      .then(gotStream)
      .catch(e => log('getUserMedia() error: ', e));
}
...
function call() {
  ...
  log('pc2: created local and remote peer connection objects');

  window.localStream.getTracks().forEach(track => pc1Local.addTrack(track, window.localStream));
  log('Adding local stream to pc1Local');
  pc1Local
  ...
  log('Adding local stream to pc2Local');
...
function handleCandidate(candidate, dest, prefix, type) {
  ...
  log(`${prefix}New ${type} ICE candidate: ${candidate ? candidate.candidate : '(null)'}`);
}
